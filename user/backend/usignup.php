...
if($password!=$cpass)
{
	...
	header("refresh:0;url=usignup.html");
}

else
{
$user_check="SELECT * FROM usignup WHERE username='$username' or email='$email'";
if(mysqli_num_rows(mysqli_query($con,$user_check)) > 0)
{
	echo "<script>alert('Email or Username exits already...')</script>";
	header("refresh:0;url=../html/usignup.html");
}
else
{
    $query="INSERT INTO usignup(username,email,password) VALUES('$username','$email','$password')";
